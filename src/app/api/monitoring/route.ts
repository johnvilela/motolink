...
import { monitoringQuerySchema } from "@/modules/monitoring/monitoring-types";
...
export async function GET(request: NextRequest) {
  ...
  const parsed = monitoringQuerySchema.safeParse({ ...params, branchId });

  if (!parsed.success) {
    return NextResponse.json({ error: "Parâmetros inválidos", details: z.treeifyError(parsed.error) }, { status: 400 });
  }

  const result = await monitoringService().getDaily(parsed.data);
