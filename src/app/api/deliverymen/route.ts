...
import { deliverymanListQuerySchema } from "@/modules/deliverymen/deliverymen-types";
...
export async function GET(request: NextRequest) {
  ...
  const parsed = deliverymanListQuerySchema.safeParse({ ...params, branchId });

  if (!parsed.success) {
    return NextResponse.json({ error: "Parâmetros inválidos", details: z.treeifyError(parsed.error) }, { status: 400 });
  }

  const result = await deliverymenService().listAll(parsed.data);
